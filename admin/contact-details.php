...
// if session is valid
  ...
  if (isset($_GET['cid'])) {
    $formid = $_GET['cid'];
    $isread = 1;
    $sql = "update tblcontactdata set Is_Read=? where id=?";
    $query = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($query, "ii", $isread, $formid);
  ...
  if (isset($_POST['submit'])) {
    $formid = $_GET['cid'];
    $remark = $_POST['remark'];
    $sql = "INSERT INTO tbladminremarks(contactFormId,adminRemark) VALUES(?,?)";
    $query = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($query, "is", $formid, $remark);
    mysqli_stmt_execute($query);
        ...
        <div class="content-body">
          <section class="card">
            <div id="invoice-template" class="card-body">
              ...
              $sql = "SELECT tblcontactdata.FullName,tblcontactdata.PhoneNumber,tblcontactdata.UserIp,tblcontactdata.EmailId,tblcontactdata.Subject,tblcontactdata.Message,tblcontactdata.PostingDate,tblcontactdata.id from tblcontactdata where tblcontactdata.id=?";
              $query = mysqli_prepare($con, $sql);
              mysqli_stmt_bind_param($query, "i", $fid);
              mysqli_stmt_execute($query);
              $resultObj = mysqli_stmt_get_result($query);
              $cnt = 1;
              if ($resultObj && mysqli_num_rows($resultObj) > 0):
                      ...
                      <div class="table-responsive col-sm-12">
                        <table class="table">
                          ...
                          <tbody>
                            ...
                            <?php $stmt = "SELECT tbladminremarks.adminRemark,tbladminremarks.remarkDate from tbladminremarks where contactFormId=?";
                            $query1 = mysqli_prepare($con, $stmt);
                            mysqli_stmt_bind_param($query1, "i", $fid);
                            mysqli_stmt_execute($query1);
                            $resultss = mysqli_stmt_get_result($query1);
                            ...
                            ?>
                              <tr>
                                ...
                                <td>
                                  <p><?php echo htmlentities($results['adminRemark']); ?>
                                    <br />
