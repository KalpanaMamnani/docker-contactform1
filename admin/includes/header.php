          ...
          <li class="dropdown dropdown-notification nav-item">
            <a class="nav-link nav-link-label" href="#" data-toggle="dropdown"><i class="ficon ft-bell"></i>
              <span class="badge badge-pill badge-default badge-danger badge-default badge-up badge-glow">
                <?php
                $isread = 1;
                $sql = "SELECT id from tblcontactdata where Is_Read!=?";
                $query = mysqli_prepare($con, $sql);
                mysqli_stmt_bind_param($query, "i", $isread);
                mysqli_stmt_execute($query);
                $results = mysqli_stmt_get_result($query);
                echo $totalcount = $results ? mysqli_num_rows($results) : 0;
                ?>
            ...
            <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
              ...
              <li class="scrollable-container media-list w-100">
                <?php
                $isread = 1;
                $sql = "SELECT FullName,PostingDate,id from tblcontactdata where Is_Read!=?";
                $query = mysqli_prepare($con, $sql);
                mysqli_stmt_bind_param($query, "i", $isread);
                mysqli_stmt_execute($query);
                $results = mysqli_stmt_get_result($query);
                      ...
                      <div class="media">
                        ...
                        <div class="media-body">
                          ...
                          <p class="notification-text font-small-3 text-muted">
                            <a href="contact-details.php?cid=<?php echo htmlentities($result['id']); ?>"><?php echo htmlentities($result['FullName']); ?></a>
                          </p>
                          <small>
