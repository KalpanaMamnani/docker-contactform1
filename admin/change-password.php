...
// if session is valid
  ...
  if (isset($_POST['update'])) {
    ...
    $sql = "SELECT Password FROM tbladmin WHERE UserName=?";
    $query = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($query, "s", $username);
    mysqli_stmt_execute($query);
    mysqli_stmt_bind_result($query, $dbpass);
    if (mysqli_stmt_fetch($query) && password_verify($currentpassword, $dbpass)) {
      $updateSql = "update tbladmin set Password=? where UserName=?";
      $chngpwd1 = mysqli_prepare($con, $updateSql);
      mysqli_stmt_bind_param($chngpwd1, "ss", $newpassword, $username);
      mysqli_stmt_execute($chngpwd1);
